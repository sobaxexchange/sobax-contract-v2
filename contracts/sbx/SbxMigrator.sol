...
contract SbxMigrator is ReentrancyGuard, ISbxMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "SbxMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "SbxMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "SbxMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "SbxMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "SbxMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "SbxMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "SbxMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "SbxMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "SbxMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "SbxMigrator: token not whitelisted");
        require(_tokenAmount > 0, "SbxMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "SbxMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(sbxPrice);
        require(mintAmount > 0, "SbxMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "SbxMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "SbxMigrator: invalid tokenA");
            require(tokenB != address(0), "SbxMigrator: invalid tokenB");
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "SbxMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "SbxMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "SbxMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "SbxMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("SbxMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "SbxMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "SbxMigrator: invalid _action");
