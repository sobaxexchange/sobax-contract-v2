...
contract Timelock is ITimelock {
    ...
    function initSblpManager() external onlyAdmin {
        ISblpManager _sblpManager = ISblpManager(sblpManager);

        IMintable sblp = IMintable(_sblpManager.sblp());
        sblp.setMinter(sblpManager, true);

        IUSDG usdg = IUSDG(_sblpManager.usdg());
        usdg.addVault(sblpManager);

        IVault vault = _sblpManager.vault();
        vault.setManager(sblpManager, true);
    }

    function initRewardRouter() external onlyAdmin {
        IRewardRouterV2 _rewardRouter = IRewardRouterV2(rewardRouter);

        IHandlerTarget(_rewardRouter.feeSblpTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(_rewardRouter.stakedSblpTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(sblpManager).setHandler(rewardRouter, true);
    }
