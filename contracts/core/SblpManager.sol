...
contract SblpManager is ReentrancyGuard, Governable, ISblpManager {
    ...
    uint256 public constant SBLP_PRECISION = 10 ** 18;
    ...
    constructor(address _vault, address _usdg, address _sblp, address _shortsTracker, uint256 _cooldownDuration) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdg = _usdg;
        sblp = _sblp;
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external override onlyGov {
        require(shortsTrackerAveragePriceWeight <= BASIS_POINTS_DIVISOR, "SblpManager: invalid weight");
    ...
    function getPrice(bool _maximise) external view returns (uint256) {
        ...
        uint256 supply = IERC20(sblp).totalSupply();
        return aum.mul(SBLP_PRECISION).div(supply);
    }
